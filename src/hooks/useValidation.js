...
import { collection, query, onSnapshot, orderBy, limit, documentId} from "firebase/firestore";
...
const useValidation = (name, apellidoPaterno, apellidoMaterno, selectedCarrera) => {
  const nameRegex = /^[a-zA-ZÀ-ÿ\s]{1,30}$/,
    ...
    function handleBlur(e) {
      ...
      useEffect(() => {
        const q = query(collection(db, "alumnos"),orderBy("numero","desc"), limit(1));
  ...
  return { errorName, errorApellidoPaterno, errorApellidoMaterno, isButtonDisabled, codigoNuevo, numeroNuevo, handleBlur};
};
